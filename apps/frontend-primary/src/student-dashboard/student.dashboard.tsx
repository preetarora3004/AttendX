import { LogOut, BookOpen, Clock, Calendar } from "lucide-react"
...
export function DashBoard() {
    ...
    const { user, loadUser } = store(useShallow((s) => ({
        user: s.user,
        loadUser: s.setUser
    })));
    ...
    const timetable = [
        { day: 'Monday', classes: ['Data Structures (10:00-11:30 AM)', 'AI & ML (1:00-2:30 PM)'] },
        { day: 'Tuesday', classes: ['Web Development (2:00-3:30 PM)', 'Linear Algebra (11:00 AM-12:30 PM)'] },
        { day: 'Wednesday', classes: ['Data Structures (10:00-11:30 AM)', 'AI & ML (1:00-2:30 PM)'] },
        { day: 'Thursday', classes: ['Web Development (2:00-3:30 PM)', 'Linear Algebra (11:00 AM-12:30 PM)'] },
                    ...
                    <div className="md:col-span-2 shadow-lg p-8 shadow-blue-100 hover:shadow-lg/50 transition-all duration-500 rounded-2xl bg-white">
                        <div className="flex items-start justify-between mb-6">
                            <div className="flex gap-6 flex-1">
                                <img className="w-24 h-24 rounded-xl object-cover border-2 border-blue-200">
                                </img>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-0 divide-y md:divide-y-0 md:divide-x divide-[#f0f5ff]">
                        {timetable.map((daySchedule, idx) => (
                            <div key={daySchedule.day} className="p-6">
                                <h4 className="font-bold text-[#132139] mb-4 text-lg">{daySchedule.day}</h4>
                                {daySchedule.classes.length > 0 ? (
                                    <div className="space-y-3">
                                        {daySchedule.classes.map((cls, cidx) => (
                                            ...
                                            >
                                                <p className="text-sm font-medium text-[#132139]">{cls}</p>
                                            </div>
                                        ))}
                                    </div>
                                ) : (
